        ...
        try:
            ...
            for idx, data in enumerate(restaurants_data):
                ...
                restaurant = Restaurant(
                    ...
                    description=None,  # Will be generated from comments
                ...
                # Generate description from first comment if available
                comments = data.get("comments", [])
                if comments:
                    first_positive = next(
                        (c for c in comments if c.get("rating", 0) >= 4),
                        None
                    )
                    if first_positive:
                        text = first_positive.get("text", "")
                        # Take first 300 chars
                        if text:
                            restaurant.description = text[:300] + "..." if len(text) > 300 else text
